...
/**
 * Makes contract-service an independent OAuth2 resource server (see
 * docs/AUTH_IMPLEMENTATION_PLAN.md sections 8 and 12). It validates the JWT itself rather
 * than trusting the gateway or any {@code X-User-*} header — a request that reaches port
 * 8081 directly is authenticated and authorized exactly as one arriving via the gateway.
 *
 ...
 * the {@code @PreAuthorize} rules on the controllers are active exactly when these
 * chains enforce and are inert when the kill switch is thrown.
 *
 * <p>Kafka listeners run outside any HTTP request and are unaffected by these chains.
 */
